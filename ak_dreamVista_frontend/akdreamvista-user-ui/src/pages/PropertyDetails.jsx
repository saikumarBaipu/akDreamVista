...
export default function PropertyDetails() {
  ...
  const fetchDetails = async () => {
    try {
      const res = await fetch(`http://3.237.234.18:8080/api/properties/${id}`, {
        headers: {
  ...
  const handlePayment = async (p) => {
    ...
    try {
      const res = await fetch("http://3.237.234.18:8080/api/payment/create-order", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
      ...
      const options = {
        ...
        handler: async function (response) {
          const verifyRes = await fetch("http://3.237.234.18:8080/api/payment/verify", {
            method: "POST",
            headers: { 
              "Content-Type": "application/json",
              "Authorization": `Bearer ${token}`
